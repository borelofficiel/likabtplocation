export const transportEngins = [

  // 🚐 VAN
  {
    id: 1,
    name: "Van MAXUS 16 Places",
    category: "Transport",
    price: "À partir de /jour",
    specs: "16 places | Diesel",
    quantity: 2,
    img: "/images/TRANSPORT/VanMAXUS16Places.jpg"
  },
  {
    id: 2,
    name: "Van TOYOTA 14 Places",
    category: "Transport",
    price: "À partir de /jour",
    specs: "14 places | Diesel",
    quantity: 2,
    img: "/images/TRANSPORT/VanTOYOTA14Places.jpg"
  },

  // 🚗 PICK UP - Voiture de Liaison 
  {
    id: 3,
    name: "PICK up NISSAN HARD BODY",
    category: "Transport",
    price: "À partir de /jour",
    specs: "Pick UP",
    quantity: 1,
    img: "/images/TRANSPORT/PICKupNISSANHARDBODY.jpg"
  },
  {
    id: 4,
    name: "Pick up TOYOTA HILUX",
    category: "Transport",
    price: "À partir de /jour",
    specs: "Pick UP",
    quantity: 2,
    img: "/images/TRANSPORT/PickupTOYOTAHILUX.jpg"
  },

  // 🚌 MINI CAR
  {
    id: 5,
    name: "MINI CAR 32 Places",
    category: "Transport",
    price: "À partir de /jour",
    specs: "32 places | Diesel",
    quantity: 1,
    img: "/images/TRANSPORT/MINICAR32Places.jpg"
  },

  // 🚚 MINI BENNE
  {
    id: 6,
    name: "KIA Mini benne 6 roues",
    category: "Transport",
    price: "À partir de /jour",
    specs: "KIA Mini benne 6 roues",
    quantity: 1,
    img: "/images/TRANSPORT/KIAMinibenne6roues.jpeg"
  },

  // 🚛 CAMION PLATEAU
  {
    id: 7,
    name: "Camion plateau Sémi-remorque DIBEX",
    category: "Transport",
    price: "À partir de /jour",
    specs: "Camion plateau",
    quantity: 2,
    img: "/images/TRANSPORT/Sémi-remorqueDIBEX.jpg"
  },
  {
    id: 8,
    name: "Camion plateau Tracteur CHENGLONG",
    category: "Transport",
    price: "À partir de /jour",
    specs: "Camion plateau",
    quantity: 2,
    img: "/images/TRANSPORT/TracteurCHENGLONG.jpg"
  },

  // 🚧 BENNE 10 ROUES
  {
    id: 9,
    name: "Benne 10 roues",
    category: "Transport",
    price: "À partir de /jour",
    specs: "SINOTRUK Benne 10 roues",
    quantity: 4,
    img: "/images/TRANSPORT/SINOTRUKBenne10roues.jpg"
  }

];